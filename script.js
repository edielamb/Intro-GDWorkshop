(() => {
  ...
  const connections = [
    ['mixed media', 'programming'], ['mixed media', 'the machine'],
    ...
    ['the machine', 'wiring'], ['touchdesigner', 'installation work'],
    ['installation work', 'wiring'], ['coding', 'programming'],
    ['programming', 'the machine'], ['web design', 'programming'],
    ['web design', 'coding'], ['installation work', 'design beyond the screen'],
  ];
  ...
  function measure() {
    ...
    nodes = words.map((element, index) => {
      ...
      return {
        element, name: text.textContent.trim(), index,
  ...
  function attachment(from, to) {
    ...
    const distance = Math.min(
      (from.halfWidth + padding) / (Math.abs(dx) || .001),
      (from.halfHeight + padding) / (Math.abs(dy) || .001),
    );
  ...
  function draw() {
    ...
    for (const node of nodes) {
      ...
      const dx = moving ? scale * (28 * Math.sin(time * .34 + phase) + 10 * Math.sin(time * .61 + phase * 1.7)) : 0;
      const dy = moving ? scale * (22 * Math.sin(time * .29 + phase * 1.3) + 8 * Math.cos(time * .53 + phase)) : 0;
      node.x = Math.max(node.halfWidth + 12 * scale, Math.min(width - node.halfWidth - 12 * scale, node.baseX + dx));
      node.y = Math.max(170 * scale, Math.min(height - node.halfHeight - 18 * scale, node.baseY + dy));
...
(() => {
  ...
  window.addEventListener('wheel', event => {
    ...
    const current = sections.find(section => y >= section.offsetTop - 2 && y < section.offsetTop + section.offsetHeight - 2);
    ...
    if (current && current.offsetHeight > innerHeight + 2) {
      ...
      if ((direction > 0 && y < bottom - 2) || (direction < 0 && y > current.offsetTop + 2)) return;
    }
    const boundaries = sections.flatMap(section => {
      const top = section.offsetTop;
      return section.offsetHeight > innerHeight + 2 ? [top, top + section.offsetHeight - innerHeight] : [top];
    });
    const target = direction > 0 ? boundaries.find(top => top > y + 2) : boundaries.findLast(top => top < y - 2);
    if (target === undefined) return;
    event.preventDefault();
    accumulated = Math.sign(accumulated) === direction ? accumulated + delta : delta;
...
(() => {
  ...
  const observer = new IntersectionObserver(entries => {
    for (const entry of entries) {
      if (entry.intersectionRatio >= .15) entry.target.classList.add('is-visible');
...
(() => {
  ...
  const paths = [
    [.5, -2, -1, 36], [-.6, 2.6, 1.4, 42], [.7, -2.8, -1.4, 39],
    [-.5, 2.2, 1, 35], [.6, -2.4, -1, 44], [-.5, 2.7, 1.4, 38],
  ];
  title.replaceChildren(...[...word].map((character, index) => {
  ...
  new IntersectionObserver(([entry]) => {
    if (entry.intersectionRatio >= .35) section.classList.add('letters-floating');
    else if (!entry.isIntersecting) section.classList.remove('letters-floating');
